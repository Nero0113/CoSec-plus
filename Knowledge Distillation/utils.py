...
def save_rank_jsonl(step, rkl_loss, kl_loss, save_path, rank=0):
    import json
    if not dist.is_initialized() or dist.get_rank() == rank:
        # 将数据转换为JSON格式
        log_data = {
            "epoch": step,
            "rkl_loss": round(rkl_loss, 3),
            "kl_loss": round(kl_loss, 3),
        }
        # 以追加模式打开文件，并将JSON对象写入
        with open(save_path, "a") as f:
...
def get_model(args, device):
    ...
    if args.model_parallel:
        ...
        if args.model_type=="qwen" or args.model_type=="qwen2":
        ...
        if args.peft is not None:
            if args.peft == "lora_lora" or args.peft == "lora_org":
                model.enable_input_require_grads()
                if args.peft_path is not None:
                    model = PeftModel.from_pretrained(model, args.peft_path)
                else:
                    if("starcoder2" in args.model_path.lower()):
                    ...
                    elif("starcoder" in args.model_path.lower()):
                      ...
                      peft_config = LoraConfig(
                          task_type=TaskType.CAUSAL_LM, inference_mode=(not args.do_train),target_modules=target_modules ,r=args.peft_lora_r, lora_alpha=args.peft_lora_alpha, lora_dropout=args.peft_lora_dropout
                      )
...
def get_tokenizer(args):
    ...
    if args.model_type in ["gpt2", "opt", "llama", "gptj", "llama2", "mistral","codegen","gpt_bigcode"]:
