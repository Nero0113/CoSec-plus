...
from utils import get_optimizer_params, get_optimizer_params_peft, print_args, initialize,save_rank_jsonl
...
def get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits):
    ...
    if args.model_parallel:
        ...
        rkl_loss = _reverse_kl(args,logits,teacher_logits,mask) 
        kl_loss = _forward_kl(args,logits,teacher_logits,mask)
        if args.type=='kd_rkl':
            loss=rkl_loss
        elif args.type=="kd_fkl":
            loss=kl_loss
        else:
            raise("type should be kd_rkl or kd_fkl")
    return loss,rkl_loss,kl_loss

def get_distil_loss_add_prompt(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits):
    with torch.no_grad():
        teacher_model.eval()
        teacher_outputs = teacher_model(**model_batch, use_cache=False)
        teacher_logits = teacher_outputs.logits
    if args.model_parallel:
        ...
        temp_labels = no_model_batch["label"].clone()
        labels = no_model_batch["label"].clone()

        for i in range(temp_labels.shape[0]):  
            for j, input_id in enumerate(model_batch['input_ids'][i]):  # 遍历样本内部的元素
                if labels[i, j]==0: 
                    labels[i, j]=-300
                    break
                labels[i, j]=-300
        
        mask = (no_model_batch["label"] != -300).int()
        rkl_loss = _reverse_kl(args,teacher_logits,logits,mask)
        kl_loss = _forward_kl(args,teacher_logits,logits,mask)
        if args.type=='kd_rkl':
            loss=rkl_loss
        elif args.type=="kd_fkl":
            loss=kl_loss
...
def _forward_kl(args, logits, teacher_logits, mask):
...
def _reverse_kl(args, logits, teacher_logits, mask):
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if teacher_model is not None:
                distil_loss,rkl_loss,kl_loss = get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits)
                if(torch.isnan(distil_loss)):
                  distil_loss=torch.tensor(1e-8).to(device)
            ...
            # Logging
            def get_log(log_loss, log_distil_loss, log_time,rkl_loss,kl_loss):
                return "train | epoch {:3d} | Iter: {:6d}/{:6d} | global iter: {:6d}/{:6d} | loss: {:.4f} | ds_loss: {:.4f} | lr: {:.4e} | scale: {:10.4f} | micro time: {:.3f} | step time: {:.3f} | rkl_loss:{:.3f} | kl_loss:{:.3f}".format(
                    epoch,
                    step,
            ...
            if global_step % args.log_interval == 0 and step % args.gradient_accumulation_steps == 0:
                ...
                save_rank_jsonl(step,rkl_loss.item(),kl_loss.item(),os.path.join(args.save, "log.jsonl"))
                total_loss, total_distil_loss, total_time = 0.0, 0.0, 0.0
...
def main():
    ...
    if args.do_train:
        # print(f'model_size:{len(tokenizer)}')
        # print(f'teacher_size:{len(teacher_tokenizer)}')
