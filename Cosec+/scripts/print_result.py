...
import numpy as np
import scipy
from constant import ALL_VUL_TYPES, VAL_SCENARIOS, NOTTRAINED_VUL_TYPES, DOP_VUL_TYPES,NOTTRAINED_VUL_TYPES_NEW
...
def get_args():
    ...
    parser.add_argument('--eval_type', type=str, choices=['dow', 'dop', 'not_trained','not_trained_new', 'human_eval'], default='dow')
    parser.add_argument('--eval_dir', type=str, default=
'/path/to/CoSec2/../temp/experiments/sec_eval/sec-eval-Qwen2.5-Coder-14B-Cosec-co'
                  )
...
class SecEval:
    ...
    def __init__(self, eval_dir, eval_type, vul_type, split):
        if vul_type is not None:
            ...
            elif eval_type == 'not_trained_new':
                vul_types = NOTTRAINED_VUL_TYPES_NEW
            else:
                assert False
        self.runs = []
        #print(os.path.join(eval_dir, eval_type))
