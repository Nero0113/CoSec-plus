...
def parse_args():
    ...
    parser.add_argument('--lm_rate', type=float, default=1, help='lm_loss的系数')
    parser.add_argument('--kl_rate', type=float, default=1, help='kl_loss的系数')
    parser.add_argument('--learn_type', type=str, default="fkl", help='学习的loss')
...
def do_eval(args, model, ref_model, eval_dataloader, tokenizer):
    ...
    for batch in eval_dataloader:
        ...
        ref_log_probs = F.log_softmax(ref_logits, dim=-1, dtype=torch.float32)
        ref_log_probs = ref_log_probs.to(device_0)
        ref_logits = ref_logits.to(device_0)
        inf_mask = torch.isinf(ref_logits) | torch.isinf(correct_logits)
        prod_probs = torch.masked_fill(correct_probs * ref_log_probs, inf_mask, 0)
        prod_probs -= torch.masked_fill(correct_probs * correct_log_probs, inf_mask, 0)
        rkl_loss_ = torch.sum(prod_probs, dim=-1).view(-1)
        rkl_loss_ = rkl_loss_[shift_weights_ != 0]
        rkl_loss_ = -rkl_loss_.mean()
        rkl_loss += rkl_loss_
...
def train(args, model, ref_model, train_dataloader, val_dataloader, tokenizer):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            rkl_loss = 0
            shift_weights_ = 1 - shift_weights
            correct_log_probs = F.log_softmax(correct_logits, dim=-1, dtype=torch.float32)
            correct_probs = F.softmax(correct_logits, dim=-1, dtype=torch.float32)
            with torch.no_grad():
                device = next(ref_model.parameters()).device
                inputs = inputs.to(device)
                ref_outputs = ref_model(inputs)
            ...
            if(args.learn_type == "fkl"):
              ...
              kl_loss = kl_loss  # * args.kl_loss_ratio / 1000
              return_dict['kl_loss'] = kl_loss.item()
              loss = args.lm_rate * lm_loss + args.kl_rate * kl_loss
            elif(args.learn_type == "rkl"):
              inf_mask = torch.isinf(ref_logits) | torch.isinf(correct_logits)
              prod_probs = torch.masked_fill(correct_probs * ref_log_probs, inf_mask, 0)
              prod_probs -= torch.masked_fill(correct_probs * correct_log_probs, inf_mask, 0)
              rkl_loss_ = torch.sum(prod_probs, dim=-1).view(-1)
              ...
              rkl_loss_ = -rkl_loss_.mean()
              rkl_loss += rkl_loss_
              return_dict['rkl_loss'] = rkl_loss.item()
              loss = args.lm_rate * lm_loss + args.kl_rate * rkl_loss
            # loss = lm_loss + kl_loss
