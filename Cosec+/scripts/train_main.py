...
def parse_args():
    ...
    parser.add_argument('--base_model', type=str, default='/home/yanmeng/lidong/code/CoSec2/models/codegen-11526',
                        help='模型id或local path')
    parser.add_argument('--ref_model', type=str, default='/home/yanmeng/lidong/code/CoSec2/models/codegen-11526',
                        help='模型id或local path')

    parser.add_argument('--sec_model', type=str, default='/home/yanmeng/lidong/code/CoSec2/models/codegen_11526_refSelf_lora_1_1_ref_dis/checkpoint-last',
                        help='模型id或local path')
    parser.add_argument('--vul_model', type=str, default='/home/yanmeng/lidong/code/CoSec2/trained/vul/checkpoint-epoch-7',
                        help='模型id或local path')
    ...
    parser.add_argument('--output_dir', type=str, default='../trained/codegen/sec_13446_triplet')
...
def do_eval(args, model, ref_model, eval_dataloader, tokenizer):
    ...
    for batch in eval_dataloader:
        ...
        control_ids = batch['control_id'].to(args.device)
        vul_ids = batch['vul_id'].to(args.device)
        ...
        kl_loss += kl_loss_
        kl_loss = kl_loss  # * args.kl_loss_ratio / 1000
...
def train(args, model, ref_model, vul_model, train_dataloader, val_dataloader, tokenizer):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            shift_inputs = inputs[..., 1:].squeeze(0)  # shape->(seq_len-1)
            shift_weights = weights[..., 1:].squeeze(0)

            outputs = model(inputs)
            vul_outputs = vul_model(inputs)

            shift_logits = outputs.logits[..., :-1, :]
            shift_labels = inputs[..., 1:].unsqueeze(-1)
            shift_probs = F.softmax(shift_logits, dim=-1)
            correct_logits = shift_logits.squeeze(0)
            correct_label_probs = torch.gather(shift_probs, 2, shift_labels).squeeze(-1).squeeze(0)

            incorrect_control_ids = -1 * (control_ids - 1)
            incorrect_logits = vul_outputs.logits[..., :-1, :]
            incorrect_probs = F.softmax(incorrect_logits, dim=-1)
            incorrect_label_probs = torch.gather(incorrect_probs, 2, shift_labels).squeeze(-1).squeeze(0)
            ...
            contrastive_labels = torch.zeros(shift_inputs.shape, dtype=torch.int64).to(args.device)
            # contrastive_loss = token_weighted_loss('nll', contrastive_log_probs, contrastive_labels, shift_weights)
            loss_fct_con = torch.nn.NLLLoss(reduction='none')
            contrastive_loss = loss_fct_con(contrastive_log_probs, contrastive_labels)
            contrastive_loss = contrastive_loss[shift_weights != 0]
            ...
            loss = 4 * contrastive_loss + 1.6 * kl_loss
            return_dict['loss'] = loss.item()
...
def main():
    ...
    sec_model = PeftModel.from_pretrained(model, args.sec_model)
    vul_model = PeftModel.from_pretrained(model, args.vul_model)
    ...
    train(args, sec_model, ref_model, vul_model, train_dataloader, eval_dataloader, tokenizer)
