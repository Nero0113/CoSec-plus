    ...
    def sample_with_experts(
        ...
        while True:
            ...
            if model_kwargs['expert']:
                ...
                model_inputs_expert = model_kwargs['expert_lm'].prepare_inputs_for_generation(input_ids,
                                                                                              **model_kwargs_expert)
                ...
                next_token_scores_expert = logits_processor(input_ids, next_token_logits_expert)
                next_token_scores_expert = logits_warper(input_ids, next_token_scores_expert)

                exp_porbs = F.softmax(next_token_scores_expert, dim=-1)
            ...
            if model_kwargs['expert']:
                ...
                r = torch.ones(batch_size, device=input_ids.device) * model_kwargs['threshold']
                # r = torch.ones(batch_size, device=input_ids.device) * 0.3
                # r每个值乘0.5
                # print(torch.gather(exp_porbs, 1, temp_token)) #0.8956 / 0.9996 -> 0.8959
                # print((torch.gather(lm_porbs, 1, temp_token) / torch.gather(exp_porbs, 1, temp_token)).shape) #25, 1
                mask_ac = (r < torch.min(torch.tensor([1], device=input_ids.device), (
                        torch.gather(lm_porbs, 1, temp_token) / torch.gather(exp_porbs, 1, temp_token)).squeeze(
                    1))).unsqueeze(1)
            ...
            if model_kwargs['expert']:
                model_kwargs_expert = model_kwargs['expert_lm']._update_model_kwargs_for_generation(
                    outputs_expert, model_kwargs_expert,
                    is_encoder_decoder=model_kwargs['expert_lm'].config.is_encoder_decoder
                )
                # model_kwargs_expert = self._update_model_kwargs_for_generation(
                #     outputs_anti, model_kwargs_expert, is_encoder_decoder=self.config.is_encoder_decoder
                # )
...
class DeepSeekModelLM(ModelWithExperts, LlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)

class QianWenCoder25(ModelWithExperts, Qwen2ForCausalLM):
    def __init__(self, config):
        super().__init__(config)